...
public class BinarySearchTree extends BinaryTree implements Iterable {
    ...
    private ArrayList<Node<String,String>> inf = new ArrayList<>();
    ...
    private void fillArrayListLevelOrder(Node root)
    {
        ...
        while (queue.size() != 0)
        {
            arrLevelOrder.add(queue.peek());
           // if(isArrListUnique(inf,queue.peek().getCityName()))
            inf.add(new Node<>(queue.peek().getData(),queue.peek().getCityName()));
    ...
    private boolean isArrListUnique(ArrayList<Node<String,String>> arr, String cityName)
    {
        for(int i = 0; i < arr.size(); ++i)
        {
            if(arr.get(i).getCityName().equals(cityName))
                return false;
        }
        return true;
    }
    public ArrayList<Node<String,String>> getInfArrayList()
    {
        return inf;
    }
